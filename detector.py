...
def detect_potential_rebound(
    curr_confidence_score, n_prev_scores, centroid_pix_locs, intensities, threshold
):
    highest_intensity_loc = centroid_pix_locs[np.argmax(intensities)]
    return (
        (
            np.all(curr_confidence_score < n_prev_scores)
            and curr_confidence_score < threshold
        ),
        highest_intensity_loc,
    )
...
def detect_rebound(
    ...
    potential_rebound_percent = (
        np.count_nonzero(previous_rebounds) / np.shape(previous_scores)[0]
    )

    # check if the current score is greater than all m previous scores, m=hyperparameter
    curr_score_above_m_prev_scores = (
        previous_scores[curr_confidence_score <= previous_scores].size
        == previous_scores.size
    )
    ...
    # print("curr confidence above: ", curr_confidence_score > threshold)
    if (
        (curr_confidence_score < threshold)
        and (potential_rebound_percent > 0.12)
        and (curr_score_above_m_prev_scores and curr_score_window_check)
    ):
...
if __name__ == "__main__":
    ...
    curr_blob_centroid_pix_locs = np.array([[20, 10], [100, 90], [500, 450],])
