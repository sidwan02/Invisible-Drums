...
def setup_path(args):
    ...
    logPath = os.path.join(
        "../logs/",
        f"{dt_string}-dataset_{dataset}-{flow_to_rgb_text}-"
...
def setup_dataset(args):
    ...
    folders = [
        os.path.basename(x)
        for x in gb.glob(os.path.join(basepath, "Flows_gap1/{}/*".format(res)))
    ]
    flow_dir = {}
    for pair in pair_list:
        ...
        for f in folders:
            ...
            flowpair = np.array(
                [[os.path.join(path1, i), os.path.join(path2, i)] for i in intersect]
            )
            flowpairs += [flowpair]
    ...
    trn_dataset = FlowPair(
        data_dir=data_dir,
        resolution=resolution,
        to_rgb=args.flow_to_rgb,
        with_rgb=False,
        with_gt=with_gt,
    )

    val_dataset = FlowEval(
