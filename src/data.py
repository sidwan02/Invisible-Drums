...
def readFlow(sample_dir, resolution, to_rgb):
    print(f"in readFlow with sample_dir: {sample_dir}")
...
class FlowPair(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f"in __getitem__ with flos: {flos}")

        for flo in flos:
            flosplit = flo.split(os.sep)
            rgb_dir = os.path.join(
                self.data_dir[1], flosplit[-2], flosplit[-1]
            ).replace(".flo", ".jpg")
            gt_dir = os.path.join(self.data_dir[2], flosplit[-2], flosplit[-1]).replace(
                ".flo", ".png"
            )
        ...
        out = (
            ...
            else np.stack([np.stack(flows, 0), np.stack(rgbs, 0)], -1)
        )
...
class FlowEval(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for gap, _gap in zip(self.gaps, self.neg_gaps):
            ...
            print(f"in data __getitem__ with flow_dir: {flow_dir}")
