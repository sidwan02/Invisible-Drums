...
def eval(
    ...
    with torch.no_grad():
            ...
            for i in range(masks.size()[0]):
                ...
                if not train:
                    ...
                    for mi, save_mask in enumerate(save_masks):
                        ...
                        os.makedirs(
                            os.path.join(resultsPath, category, fgp), exist_ok=True
                        )
                        cv2.imwrite(
                            os.path.join(resultsPath, category, fgp, index),
                            (save_mask * 255.0),
                        )
                    mean_mask = einops.rearrange(mean_mask, "c h w -> h w c")
                    os.makedirs(
                        os.path.join(resultsPath, category, "mean"), exist_ok=True
                    )
                    cv2.imwrite(
                        os.path.join(resultsPath, category, "mean", index),
                        (mean_mask * 255.0),
                    )
            ...
            if train:
                ...
                if np.random.random() > 0.95:
                    ...
                    grid_ri = ut.convert_for_vis(
                        recon_image, use_flow=use_flow
                    ).unsqueeze(1)
                    grid_r = ut.convert_for_vis(recons, use_flow=use_flow)
                    grid_m = (255 * torch.cat([masks, masks, masks], dim=3)).type(
                        torch.ByteTensor
                    )
        ...
        frame_single_step_mean_iou = sum(single_step_frameious) / len(
            single_step_frameious
        )
        print(
            " --> inference, time {}".format(time.time() - t),
        ...
        if train:
            ...
            writer.add_scalar(
                "IOU/val_single_step_mean", frame_single_step_mean_iou, it
            )
...
def main(args):
    ...
    print("about to initialize training for eval")
    # initialize training
