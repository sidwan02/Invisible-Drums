...
def main(args):
    ...
    while it < num_it:
        for _, sample in enumerate(trn_loader):
            ...
            entropy_loss = (
                ent_scale * -(masks * torch.log(masks + epsilon)).sum(dim=1).mean()
            )
            ...
            consistency_loss = (
                cons_scale * temporal_diff.view(-1, 2 * 2).min(1)[0].mean()
            )
            ...
            if it % log_freq == 0:
                ...
                writer.add_scalar(
                    "Loss/reconstruction", recon_loss.detach().cpu().numpy(), it
                )
                writer.add_scalar(
                    "Loss/entropy", entropy_loss.detach().cpu().numpy(), it
                )
                writer.add_scalar(
                    "Loss/consistency", consistency_loss.detach().cpu().numpy(), it
                )
                if monitor_train_iou:
            ...
            if it % eval_freq == 0 and frame_mean_iou > iou_best:
                filename = os.path.join(
                    modelPath,
                    "checkpoint_{}_iou_{}.pth".format(it, np.round(frame_mean_iou, 3)),
                )
                torch.save(
                    {
                        "iteration": it,
